        ...
        if (GFC.enabled && SG.group[GFC.group].enabled) {
            if (GF.dir) {
                ...
                grid_fill_area(GFC.x + GF.value + 1, GFC.y,
                               GFC.w - GF.value - 1, GFC.h, GFC.background);
            }
        }
    }
...
void grid_screen_refresh_ctrl(scene_state_t *ss, u8 page, u8 x1, u8 y1, u8 x2,
    ...
    for (u16 i = 0; i < GRID_BUTTON_COUNT; i++) {
        ...
        if (GBC.w == 1 && GBC.h == 1) {
            ...
            grid_fill_area_scr(GBC.x, GBC.y + 1, 1, GBC.h - 2, level + 11,
    page);
            grid_fill_area_scr(last_x, GBC.y + 1, 1, GBC.h - 2, level + 12,
    page);
            grid_fill_area_scr(GBC.x, last_y, 1, 1, level + 13, page);
            grid_fill_area_scr(GBC.x + 1, last_y, GBC.w - 2, 1, level + 14,
    page);
...
void grid_screen_refresh_led(scene_state_t *ss, u8 full_grid, u8 page, u8 x1,
    ...
    for (u8 i = 0; i < GRID_FADER_COUNT; i++) {
        if (GFC.enabled && SG.group[GFC.group].enabled) {
            if (GF.dir) {
                grid_fill_area_scr(GFC.x, GFC.y, GFC.w, GFC.h - GF.value - 1,
                                   GFC.background, page);
                grid_fill_area_scr(GFC.x, GFC.y + GFC.h - GF.value - 1, GFC.w,
                                   GF.value + 1, 15, page);
            }
            else {
    ...
    for (u16 i = 0; i < GRID_BUTTON_COUNT; i++)
        if (GBC.enabled && SG.group[GBC.group].enabled)
            grid_fill_area_scr(GBC.x, GBC.y, GBC.w, GBC.h,
                               GB.state ? 15 : GBC.background, page);
...
static void grid_screen_refresh_info(scene_state_t *ss, u8 page, u8 x1, u8 y1,
    ...
    if (page == 0) {
        for (u16 i = 1; i < 5; i++)
            for (u16 j = 1; j < 3; j++) line[0].data[i + (j << 7)] = 15;
    }
    else {
        for (u16 i = 1; i < 5; i++)
            for (u16 j = 3; j < 5; j++) line[0].data[i + (j << 7)] = 15;
    }
...
void grid_fill_area_scr(u8 x, u8 y, u8 w, u8 h, u8 level, u8 page) {
    ...
    if (level == LED_DIM) {
        for (u16 _x = x; _x < x_end; _x++) {
            for (u16 _y = y; _y < y_end; _y++) screen[_x][_y] >>= 1;
        }
    }
    else if (level == LED_BRI) {
        for (u16 _x = x; _x < x_end; _x++)
        ...
        for (u16 _x = x; _x < x_end; _x++)
            for (u16 _y = y; _y < y_end; _y++) screen[_x][_y] = level;
    }
}
